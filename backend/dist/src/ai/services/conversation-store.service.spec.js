...
const conversation_store_service_1 = require("./conversation-store.service");
const supabase_service_1 = require("../../supabase/supabase.service");
const ai_constants_1 = require("../constants/ai.constants");
describe('ConversationStoreService', () => {
    let service;
    let mockSupabaseClient;
    const createQueryBuilder = (resolvedData = null, resolvedError = null) => {
        const builder = {};
    ...
    beforeEach(async () => {
        ...
        service = module.get(conversation_store_service_1.ConversationStoreService);
    });
    describe('createConversation', () => {
        ...
        it('should set expires_at to creation time + 90 days', async () => {
            ...
            expectedExpiry.setDate(expectedExpiry.getDate() + ai_constants_1.CONVERSATION_EXPIRY_DAYS);
            let capturedInsert;
            const builder = {};
        ...
        it('should throw an error when Supabase insert fails', async () => {
            ...
            mockSupabaseClient.from.mockReturnValue(builder);
            await expect(service.createConversation('user-1')).rejects.toThrow('Failed to create conversation: Database error');
        });
    });
    describe('addMessage', () => {
        ...
        it('should persist an assistant message', async () => {
            ...
            countBuilder.select = jest.fn().mockReturnValue(countBuilder);
            countBuilder.eq = jest.fn().mockReturnValue(countBuilder);
            countBuilder.then = (resolve) => resolve({ count: 2, error: null });
            let callCount = 0;
            mockSupabaseClient.from.mockImplementation((table) => {
                if (table === 'conversation_messages') {
                    callCount++;
            ...
            const result = await service.addMessage('conv-1', 'assistant', 'GCB is currently trading at GHS 5.20.', 25);
        ...
        it('should throw an error when message insert fails', async () => {
            ...
            await expect(service.addMessage('conv-1', 'user', 'Hello', 5)).rejects.toThrow('Failed to add message: Insert failed');
        });
    });
    describe('getMessages', () => {
        it('should return messages in chronological order', async () => {
            ...
            builder.then = (resolve) => resolve({ data: mockMessages, error: null });
            mockSupabaseClient.from.mockReturnValue(builder);
        ...
        it('should default limit to CONVERSATION_CONTEXT_WINDOW (50)', async () => {
            ...
            expect(builder.limit).toHaveBeenCalledWith(ai_constants_1.CONVERSATION_CONTEXT_WINDOW);
        });
        ...
        it('should throw an error when query fails', async () => {
            ...
            builder.order = jest.fn().mockReturnValue(builder);
            builder.limit = jest.fn().mockReturnValue(builder);
            builder.then = (resolve) => resolve({ data: null, error: { message: 'Query failed' } });
            mockSupabaseClient.from.mockReturnValue(builder);
            await expect(service.getMessages('conv-1')).rejects.toThrow('Failed to get messages: Query failed');
        });
    });
    describe('getContextWindow', () => {
        it('should return messages as ChatMessage[] for LLM context', async () => {
            ...
            expect(result[0].tokenCount).toBeUndefined();
        });
    ...
    describe('listConversations', () => {
        it('should return conversations sorted by updated_at desc, excluding soft-deleted', async () => {
            ...
            builder.order = jest.fn().mockReturnValue(builder);
            builder.then = (resolve) => resolve({ data: mockConversations, error: null });
            mockSupabaseClient.from.mockReturnValue(builder);
        ...
        it('should throw an error when query fails', async () => {
            ...
            builder.then = (resolve) => resolve({ data: null, error: { message: 'Query failed' } });
            mockSupabaseClient.from.mockReturnValue(builder);
            await expect(service.listConversations('user-1')).rejects.toThrow('Failed to list conversations: Query failed');
        });
    });
    describe('getConversation', () => {
        ...
        it('should throw on unexpected errors', async () => {
            ...
            mockSupabaseClient.from.mockReturnValue(builder);
            await expect(service.getConversation('conv-1', 'user-1')).rejects.toThrow('Failed to get conversation: Unexpected error');
        });
    });
    describe('deleteConversation', () => {
        ...
        it('should throw an error when soft-delete fails', async () => {
            ...
            builder.eq = jest.fn().mockReturnValue(builder);
            builder.then = (resolve) => resolve({ error: { message: 'Update failed' } });
            mockSupabaseClient.from.mockReturnValue(builder);
            await expect(service.deleteConversation('conv-1', 'user-1')).rejects.toThrow('Failed to delete conversation: Update failed');
        });
    });
    describe('context window bounding', () => {
        it('should return at most 50 messages even when more exist', async () => {
            ...
            mockSupabaseClient.from.mockReturnValue(builder);
            const result = await service.getContextWindow('conv-1');
            expect(result.length).toBeLessThanOrEqual(ai_constants_1.CONVERSATION_CONTEXT_WINDOW);
            expect(builder.limit).toHaveBeenCalledWith(50);
        });
    });
});
function createChainableBuilder(resolvedData, resolvedError = null) {
    const builder = {};
    builder.insert = jest.fn().mockReturnValue(builder);
    builder.update = jest.fn().mockReturnValue(builder);
    builder.select = jest.fn().mockReturnValue(builder);
...
//# sourceMappingURL=conversation-store.service.spec.js.map