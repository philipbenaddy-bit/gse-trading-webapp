...
describe('ContextBuilderService', () => {
    ...
    beforeEach(async () => {
        gseService = {
            ...
            getLiveBySymbol: jest.fn().mockImplementation((symbol) => {
                return mockLiveStocks.find((s) => s.name.toUpperCase() === symbol.toUpperCase()) || null;
            }),
        ...
        const module = await testing_1.Test.createTestingModule({
            providers: [
                ...
                { provide: gse_service_1.GseService, useValue: gseService },
                { provide: portfolio_service_1.PortfolioService, useValue: portfolioService },
                { provide: news_aggregator_service_1.NewsAggregatorService, useValue: newsAggregatorService },
            ],
        }).compile();
    ...
    describe('extractStockSymbols', () => {
        ...
        it('should match company names', () => {
            const result = service.extractStockSymbols('How is MTN Ghana performing today?');
            expect(result).toContain('MTNGH');
        });
        it('should match multiple symbols', () => {
            const result = service.extractStockSymbols('Compare MTNGH and GCB performance');
            expect(result).toContain('MTNGH');
            expect(result).toContain('GCB');
        });
        it('should return empty array for no matches', () => {
            const result = service.extractStockSymbols('What is the weather like today?');
        ...
        it('should return empty array for null/undefined', () => {
            expect(service.extractStockSymbols(null)).toEqual([]);
            expect(service.extractStockSymbols(undefined)).toEqual([]);
        });
        it('should match CalBank by company name', () => {
            const result = service.extractStockSymbols('What is CalBank stock price?');
    ...
    describe('buildMarketContext', () => {
        it('should return market context with matched stock data', async () => {
            const result = await service.buildMarketContext('What is the price of MTNGH?');
        ...
        it('should not include requestedStock when no symbol matched', async () => {
            const result = await service.buildMarketContext('How does the stock market work?');
            expect(result).toBeDefined();
    ...
    describe('GSE_SYMBOL_MAP', () => {
        it('should contain major GSE stocks', () => {
            expect(context_builder_service_1.GSE_SYMBOL_MAP).toHaveProperty('MTNGH');
            expect(context_builder_service_1.GSE_SYMBOL_MAP).toHaveProperty('GCB');
            expect(context_builder_service_1.GSE_SYMBOL_MAP).toHaveProperty('GOIL');
            expect(context_builder_service_1.GSE_SYMBOL_MAP).toHaveProperty('CAL');
            expect(context_builder_service_1.GSE_SYMBOL_MAP).toHaveProperty('EGH');
            expect(context_builder_service_1.GSE_SYMBOL_MAP).toHaveProperty('SCB');
        });
        it('should have lowercase aliases for each symbol', () => {
            for (const [, aliases] of Object.entries(context_builder_service_1.GSE_SYMBOL_MAP)) {
    ...
    describe('buildPortfolioContext', () => {
        ...
        it('should cap holdings at 20 (PORTFOLIO_CONTEXT_MAX_HOLDINGS)', async () => {
            ...
            expect(result.holdings[0].currentValue).toBe(224);
            expect(result.holdings[19].currentValue).toBe(205);
        });
        it('should return undefined on portfolio service error', async () => {
            portfolioService.getUserPortfolio.mockRejectedValue(new Error('Database connection failed'));
            const result = await service.buildPortfolioContext('user-1', true);
            expect(result).toBeUndefined();
        });
        it('should return undefined on timeout', async () => {
            portfolioService.getUserPortfolio.mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve({
                holdings: mockPortfolioHoldings,
                summary: { totalValue: 760, totalCost: 665, totalPnl: 95, totalPnlPercent: 14.29 },
            }), 4000)));
            const result = await service.buildPortfolioContext('user-1', true);
    ...
    describe('buildNewsContext', () => {
        ...
        it('should return news articles for stock queries filtered by symbol', async () => {
            ...
            expect(newsAggregatorService.getAllNews).toHaveBeenCalledWith(expect.objectContaining({ symbol: 'MTNGH' }));
        });
        ...
        it('should return undefined when news service throws an error', async () => {
            newsAggregatorService.getTrendingNews.mockRejectedValue(new Error('News service unavailable'));
            const result = await service.buildNewsContext([], true);
            expect(result).toBeUndefined();
        });
        ...
        it('should return undefined on timeout', async () => {
            newsAggregatorService.getTrendingNews.mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve(mockNewsArticles), 4000)));
            const result = await service.buildNewsContext([], true);
            expect(result).toBeUndefined();
        }, 10000);
