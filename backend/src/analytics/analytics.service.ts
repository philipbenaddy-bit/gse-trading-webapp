...
export class AnalyticsService {
  ...
  // ── Task 21: Market Metrics ───────────────────────────────────────────────

  getMarketMetrics() {
    const stocks = this.gseService.getAllLive();
    if (!stocks.length) return this.emptyMarketMetrics();
  ...
  // ── Task 22: Sector Allocation (from equities data) ───────────────────────
  ...
  getSectorAllocation() {
    ...
    const sectorMap: Record<string, { count: number; totalCap: number; stocks: string[] }> = {};
  ...
  // ── Task 23: User Portfolio Performance ───────────────────────────────────

  async getPortfolioPerformance(userId: string, timeframe: string) {
    const client = this.supabase.getAdminClient();

    // Get user's order history to reconstruct performance
    ...
    const dataPoints: { date: string; portfolioValue: number; invested: number }[] = [];
    let invested = 0;
    let holdings: Record<string, { qty: number; avgCost: number }> = {};

    // Group orders by week for chart data
    const weekMap: Record<string, typeof orders> = {};
    orders.forEach((order) => {
      const week = order.created_at.slice(0, 10); // YYYY-MM-DD
    ...
    Object.entries(weekMap).forEach(([date, dayOrders]) => {
      dayOrders.forEach((order) => {
        ...
        if (order.side === 'buy') {
          invested += qty * price;
          if (!holdings[order.symbol]) holdings[order.symbol] = { qty: 0, avgCost: 0 };
          holdings[order.symbol].qty += qty;
          holdings[order.symbol].avgCost = price;
        } else {
          invested -= qty * price;
          if (holdings[order.symbol]) holdings[order.symbol].qty -= qty;
        }
      });

      // Estimate current value using live prices
  ...
  async getTradingStats(userId: string) {
    ...
    // Calculate win rate from sell orders
    ...
    sells.forEach((sell) => {
      ...
      if (buy) {
        const days =
          (new Date(sell.created_at).getTime() - new Date(buy.created_at).getTime()) /
          (1000 * 60 * 60 * 24);
        totalHoldDays += days;
        holdCount++;
      }
  ...
  async getExportData(userId: string, type: 'portfolio' | 'orders' | 'transactions') {
    const client = this.supabase.getAdminClient();
