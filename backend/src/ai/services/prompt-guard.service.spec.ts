import { PromptGuardService, GENERIC_REFUSAL_MESSAGE } from './prompt-guard.service';
...
describe('PromptGuardService', () => {
  ...
  describe('sanitize()', () => {
    ...
    it('should remove Unicode Cf format characters (zero-width spaces, BOM, etc.)', () => {
      // U+200B zero-width space, U+FEFF BOM, U+200D zero-width joiner
  ...
  describe('processInput()', () => {
    ...
    it('should sanitize control characters and validate', () => {
      // \x00 and \x01 are Cc control chars (removed), \t is also Cc (removed)
      // After removal: 'Hello World  Test', after collapse: 'Hello World Test'
    ...
    it('should detect injection and return generic refusal', () => {
      ...
      expect(result.error).toBe(GENERIC_REFUSAL_MESSAGE);
    });
    ...
    it('should return the same generic refusal for all injection types', () => {
      ...
      expect(roleOverride.error).toBe(GENERIC_REFUSAL_MESSAGE);
      expect(promptExtraction.error).toBe(GENERIC_REFUSAL_MESSAGE);
      expect(delimiterInjection.error).toBe(GENERIC_REFUSAL_MESSAGE);
    });

    it('should run sanitization before validation', () => {
      // Input with control chars that after removal leaves valid content
  ...
  describe('stripPii()', () => {
    ...
    describe('email detection', () => {
      ...
      it('should handle multiple different emails', () => {
        const result = service.stripPii(
          'From john@example.com to jane@test.org',
          sessionId,
        );
        expect(result).toBe(
          'From [REDACTED_EMAIL_1] to [REDACTED_EMAIL_2]',
        );
      });

      it('should handle complex email formats', () => {
        const result = service.stripPii(
          'Email: user.name+tag@sub.domain.co.gh',
          sessionId,
        );
    ...
    describe('Ghana Card detection', () => {
      ...
      it('should detect multiple Ghana Card numbers', () => {
        const result = service.stripPii(
          'Cards: GHA-123456789-0 and GHA-987654321-1',
          sessionId,
        );
        expect(result).toBe(
          'Cards: [REDACTED_ID_1] and [REDACTED_ID_2]',
        );
      });
    });
    ...
    describe('session consistency', () => {
      it('should map same PII to same anonymized ID within a session', () => {
        const result1 = service.stripPii(
          'Email: user@example.com',
          sessionId,
        );
        const result2 = service.stripPii(
      ...
      it('should use different anonymized IDs for different PII values', () => {
        const result = service.stripPii(
          'Emails: a@test.com and b@test.com',
          sessionId,
        );
        expect(result).toBe(
          'Emails: [REDACTED_EMAIL_1] and [REDACTED_EMAIL_2]',
        );
      });
      ...
      it('should maintain separate mappings for different sessions', () => {
        ...
        // Both sessions should produce [REDACTED_EMAIL_1] since it's the first email in each
        expect(result1).toBe('Email: [REDACTED_EMAIL_1]');
      ...
      it('should maintain counters per category within a session', () => {
        ...
        // Second email should be EMAIL_2, phone counter is separate
    ...
    describe('mixed PII types', () => {
      it('should handle multiple PII types in one input', () => {
        const input =
          'Contact john@example.com at +233241234567, card GHA-123456789-0';
        const result = service.stripPii(input, sessionId);
        expect(result).toBe(
          'Contact [REDACTED_EMAIL_1] at [REDACTED_PHONE_1], card [REDACTED_ID_1]',
        );
      });
  ...
  describe('clearSession()', () => {
    it('should clear session mappings so new PII gets fresh IDs', () => {
      ...
      // After clearing, same PII should get a fresh ID (starting from 1 again)
    ...
    it('should not affect other sessions', () => {
      ...
      // session2 should still have its mapping
