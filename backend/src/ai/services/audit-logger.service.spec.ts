...
describe('AuditLoggerService', () => {
  ...
  describe('logInteraction', () => {
    ...
    it('should not include raw LLM response in audit record', async () => {
      ...
      const insertedRecord = mockInsert.mock.calls[0][0];
      // Verify no content/response field exists
  ...
  describe('logSecurityEvent', () => {
    ...
    it('should truncate sanitizedInput to 2000 characters', async () => {
      ...
      const insertedRecord = mockInsert.mock.calls[0][0];
      expect(insertedRecord.sanitized_input.length).toBe(
        SECURITY_EVENT_INPUT_MAX_LENGTH,
      );
      expect(insertedRecord.sanitized_input).toBe(
        'a'.repeat(SECURITY_EVENT_INPUT_MAX_LENGTH),
      );
    });
  ...
  describe('generateDailySummary', () => {
    ...
    it('should not throw when audit log fetch fails', async () => {
      ...
      await expect(
        service.generateDailySummary(new Date('2024-01-15')),
      ).resolves.not.toThrow();
    });
  ...
  describe('retry queue', () => {
    it('should process retry queue and persist items', async () => {
      // First call fails, enqueuing the item
      ...
      // Now make insert succeed for retry
    ...
    it('should discard items after max retry attempts', async () => {
      ...
      // Process retries until max attempts exceeded
      await service.processRetryQueue(); // retry 1
      await service.processRetryQueue(); // retry 2
      await service.processRetryQueue(); // retry 3 - discarded
    ...
    it('should keep items in queue if retry fails but under max attempts', async () => {
      ...
      await service.processRetryQueue(); // retry 1 fails
